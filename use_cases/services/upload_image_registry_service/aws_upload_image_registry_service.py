...
class AwsUploadImageRegistryService(UploadImageRegistryService):
    ...
    def upload(self, image) -> None:
        ...
        os.system(
            f"docker tag {image.tag}:latest {account_id}.dkr.ecr.{region}.amazonaws.com/{image.repository}:latest"
        )
        os.system(
            f"docker push {account_id}.dkr.ecr.{region}.amazonaws.com/{image.repository}:latest"
        )
